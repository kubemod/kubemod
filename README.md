...
A large number of services and platforms are now being deployed using bespoke customized operators instead of deploying primitive Kubernetes resources through Helm charts, kustomize or kubectl.
This is all great as the operator pattern encapsulates the complexity of deploying tens of resources and boils it down to a number of domain-specific custom resources.

That said, the operator pattern introduces a challenge -- since the primitive resources generated by the operator are directly sent to Kubernetes through the Kubernetes API, this makes it impossible to apply additional customizations to those resources prior to or at the time of deployment.
...
The `patch` part of the ModRule is a [Golang template](https://golang.org/pkg/text/template/) which takes the target object as a template variable allowing for powerful declarative rules such as this one which injects a [Jaeger Agent](https://www.jaegertracing.io/docs/1.19/architecture/#agent) sidecar into any Deployment tagged with annotation `my-inject-annotation` set to `true`:
...
Here's an example of fixing an issue with Elastic Search Operator: https://github.com/elastic/cloud-on-k8s/issues/2328.
The issue is that the when the operator creates Persistent Volumes, it attaches an ownerReference to them such that they are removed after the operator removes the Elastic Search stack of resources.
...
If an object matches that `match` section, it's creation will be rejected.
This allows us to develop a system of policy ModRules which enforce certain rules for the namespace they are deployed to.

For example, here's a `Reject` ModRule which rejects the deployment of any `Deployment` or `StatefulSet` resource which does not explicitly forbid non-root containers:
