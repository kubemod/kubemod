...
func (app *KubeModWebApp) dryRunHandler(c *gin.Context) {
	...
	patched, patch, err := store.CalculatePatch(v1beta1.ModRuleOperation(dryRunOperation), dryRunNamespace, originalJSON, app.log)
	...
	rejections := store.DetermineRejections(v1beta1.ModRuleOperation(dryRunOperation), dryRunNamespace, patched, app.log)
